...
//     (c) 2014 Kevin Combriat

var add, apply, applyWith, and, areArguments, arity, arity0, arity1, arity2, arity3, assoc, attrs, bind, bitand, bitor, bitxor, bitnot,
    bitlshift, bitrshift2, bitrshift3, butfirst, butlast, call, callWith, clone, comp, complement, concat, concatMap, concatMapkv, conj,
    cons, cs, curriedFunctions, curry, curry1, cycle, dec, del, dir, dir1, div, doc, drop, dropWhile, eq, eq2, eq3, eqOne, eqZero, error,
    error1, even, every, fdrop, fget, filter, first, flatten, flip, fmap, fmapkv, fmapkvo, fmapo, fn, freduce, freducekv, freducer, get,
    gt, gte, id, inc, interleave, interpose, is, isa, isArray, isArrayLike, isBoolean, isDate, isEmpty, isFalse, isFloat, isFunction, isInt,
    isNull, isNumber, isObject, isString, isTrue, isUndefined, join, juxt, keys, last, len, log, log1, lookup, loop, lt, lte, map, mapkv,
    mapkvo, mapo, marshal, max, merge, min, mul, neq2, neq3, not, odd, or, owns, partial, pow, product, rand, randIndex, randInt, range,
    reduce, reducekv, reducer, repeat, repeatedly, reverse, second, shuffle, slice, some, sort, source, sqrt, sub, sum, takeWhile, thread,
 ...
 * @see bitrshift3
 */
exports.bitand = bitand = function bitand() {
  var args = arguments;
  var reducer = function(x, y, rest) {
    if (rest.length == 1) return x & y;
    return x & reducer(rest[0], rest[1], slice(rest, 1));
  };
  return reducer(args[0], args[1], slice(args, 1));
};
 ...
 * @see bitrshift3
 */
exports.bitor = bitor = function bitor() {
  var args = arguments;
  var reducer = function(x, y, rest) {
    if (rest.length == 1) return x | y;
    return x | reducer(rest[0], rest[1], slice(rest, 1));
 ...
 * @see bitrshift3
 */
exports.bitxor = bitxor = function bitxor() {
  var args = arguments;
  var reducer = function(x, y, rest) {
    if (rest.length == 1) return x ^ y;
    return x ^ reducer(rest[0], rest[1], slice(rest, 1));
 ...
 * @see bitrshift3
 */
exports.bitnot = bitnot = function bitnot(x) {
  return ~x;
};
 ...
 * @see bitrshift3
 */
exports.bitlshift = bitlshift = function bitlshift() {
  var args = arguments;
  var reducer = function(head, x, y) {
    if (head.length == 1) return x << y;
    return reducer(slice(head, 0, -1), head[head.length - 2], last(head)) << y;
  };
  return reducer(slice(args, 0, -1), args[args.length - 2], last(args));
};
 ...
 * @see bitrshift3
 */
exports.bitrshift2 = bitrshift2 = function bitrshift2() {
  var args = arguments;
  var reducer = function(head, x, y) {
    if (head.length == 1) return x >> y;
    return reducer(slice(head, 0, -1), head[head.length - 2], last(head)) >> y;
 ...
 * @see bitrshift2
 */
exports.bitrshift3 = bitrshift3 = function bitrshift3() {
  var args = arguments;
  var reducer = function(head, x, y) {
    if (head.length == 1) return x >>> y;
    return reducer(slice(head, 0, -1), head[head.length - 2], last(head)) >>> y;
...
(function() {
  ...
  var fs2 = ['add', 'and', 'bitand', 'bitlshift', 'bitnot', 'bitor', 'bitrshift2',
             'bitrshift3', 'bitxor', 'conj', 'div', 'eq', 'eq2', 'eq3', 'fdrop',
             'fget', 'fmap', 'fmapo', 'fmapkv', 'fmapkvo', 'gt', 'gte', 'lt', 'lte',
             'mul', 'neq2', 'neq3','or', 'sub', 'xor'];
  loop(function(_, v) {
   ...
   * 'add', 'and', 'apply', 'applyWith', 'assoc', 'bitand', 'bitlshift', 'bitnot', 'bitor', 'bitrshift2',
   * 'bitrshift3', 'bitxor''conj', 'cons', 'cycle', 'div', 'drop', 'dropWhile', 'eq', 'eq2', 'eq3', 'every',
   * 'fdrop', 'fget', 'filter', 'fmap', 'fmapo', 'fmapkv', 'fmapkvo', 'get', 'gt', 'gte', 'loop', 'lt', 'lte',
   * 'map', 'mapo', 'mapkv', mapkvo', 'mul', 'neq2', 'neq3', 'or', 'pow', 'repeat', 'repeatedly', 'some', 'sub',
   * 'takeWhile' and 'xor'
...
(function() {
   ...
   * @name versionDetails
   */
  exports.versionDetails = {major: 0, minor: 17, patch: 2};
})();
